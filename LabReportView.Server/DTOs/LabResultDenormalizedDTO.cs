...
{
    public class LabResultDenormalizedDTO
    {
        //lookup
        ...
        public static List<LabResultDenormalizedDTO> MapDataTableToSingleObject(DataTable dataRes)
        {
            LabResultDenormalizedDTO retObj = new LabResultDenormalizedDTO();
            if (dataRes != null)
            {
                string strDenormalizedData = JsonConvert.SerializeObject(dataRes);
                //Datatable contains array, we need to deserialize into list then take the first one.
                List<LabResultDenormalizedDTO> deNormalizedResult = JsonConvert.DeserializeObject<List<LabResultDenormalizedDTO>>(strDenormalizedData);

                return deNormalizedResult;
            }
            else
            {
                return new List<LabResultDenormalizedDTO>();
            }
        }
